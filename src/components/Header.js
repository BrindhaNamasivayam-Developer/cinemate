...
export const Header = () => {
    ...
    const [mode, setMode] = useState(JSON.parse(localStorage.getItem("darkSwitch")) || false);
    const nav = useNavigate();

    useEffect(() => {
        localStorage.setItem("darkSwitch", JSON.stringify(mode));
        if (mode) {
    ...
    const setHandler = (event) => {
        ...
        let searchQuery = event.target.sValue.value;   
        event.target.reset();       
        console.log(searchQuery + 'searchValue');
        return nav(`/search?q=${searchQuery}`);
    }
                ...
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    ...
                    <div className="flex md:order-2" id="mobile-view">
                        ...
                        <div className="relative hidden md:block">
                            ...
                            <form onSubmit={setHandler}>
                                <input type="text" id="search-navbar" name="sValue" className="block w-full p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search..." />
                            </form>
                        </div>
                    ...
                    <div className={`${hidden ? "hidden" : ""} items-center justify-between  w-full md:flex md:w-auto md:order-1`} id="navbar-search">
                        ...
                        <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                            ...
                            <li>
                                <NavLink to="/movie/upcoming" className={({ isActive }) => isActive ? active : inActive}>Up Coming</NavLink>
                            </li>
